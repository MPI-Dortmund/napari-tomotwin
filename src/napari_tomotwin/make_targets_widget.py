...
def _run(
    ...
    assert len(embeddings) == len(
        clusters
    ), "Cluster and embedding file are not compatible. They have a different number of embeddings"
    ...
    targets, sub_embeddings, target_locations = _make_targets(
        embeddings, clusters, avg_func=avg_method
    )
    ...
    for cluster_id in target_locations:
        ...
        if df_loc is not None and len(df_loc) > 0:
            pth_loc = os.path.join(
                output_folder, f"cluster_{cluster_id}_medoid.coords"
            )
            df_loc[["X", "Y", "Z"]].to_csv(
    ...
    for emb_i, emb in enumerate(sub_embeddings):
        pth_emb = os.path.join(
            output_folder, f"embeddings_cluster_{emb_i}.temb"
        )
...
@thread_worker
def _run_worker(
    embeddings_filepath, clusters, output_folder: str, average_method_name: str
):
...
def on_init(widget):
    ...
    def update():
        try:
            if os.path.exists(
                widget.label_layer.value.metadata["tomotwin"][
                    "embeddings_path"
                ]
            ):
                widget.embeddings_filepath.value = (
                    widget.label_layer.value.metadata["tomotwin"][
                        "embeddings_path"
                    ]
                )
                ...
                notifications.show_info(
                    "Can't find embeddings path in metadata. Please set it manually."
                )
...
def make_targets(
    ...
    worker = _run_worker(
        embeddings_filepath, clusters, output_folder, average_method_name
